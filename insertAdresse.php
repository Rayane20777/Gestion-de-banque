...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<button type='submit' name='edit_adr' class='bg-blue-600 py-2 px-8 text-white font-bold'>Edit</button>";
        echo "</form>";
        echo "</td>";
...
if (isset($_POST['update'])) {
    $id = $_POST['update_id'];
    $updatedVille = $_POST['updated_ville'];
    $updatedQuartier = $_POST['updated_quartier'];
    $updatedRue = $_POST['updated_rue'];
    $updatedCodePostal = $_POST['updated_code_postal'];
    $updatedEmail = $_POST['updated_email'];
    $updatedTelephone = $_POST['updated_telephone'];

    // Update the address record
    $updateAdresse = "UPDATE adresse SET ville = ?, quartier = ?, rue = ?, code_postal = ?, email = ?, telephone = ? WHERE id = ?";
    $statement = $conn->prepare($updateAdresse);
    $statement->bind_param("sssssii", $updatedVille, $updatedQuartier, $updatedRue, $updatedCodePostal, $updatedEmail, $updatedTelephone, $id);
